...
class OpenAICompatibleProvider:
    ...
    async def chat(self, messages: list[Message], options: ChatOptions | None = None) -> ChatResponse:
        ...
        response = await self._client.chat.completions.create(
            ...
            messages=self._format_message(messages, options.system),
        )

        choice = response["choices"][0]
        stop_reason = (
            "end_turn" if choice.finish_reson == "stop" else "max_toekns"
        )
        ...
        return ChatResponse(
            ...
            usage={
                "input_tokens": getattr(response.usage, "prompts_tokens", 0),
